...
<head>
    ...
    @if($config->is_dark_theme == 1)
        <link href="{{ asset('assets/css/dark-admin.css') }}" rel="stylesheet">
    @endif
...
<body>
    ...
    <div class="header">
        <div class="pull-left">
            <div class="logo"><img src="{{ asset('assets/images/logo.png') }}" alt="" /><span>Панель управления</span></div>
        ...
        <div class="pull-right p-r-15">
            <button class="disable-light" data-is-dark-theme="{{ $config->is_dark_theme == 0 ? '1' : '0' }}">{{ $config->is_dark_theme == 0 ? 'Выключить свет' : 'Включить свет' }}</button>
            <ul>
                <li class="header-icon dib"><img class="avatar-img" src="{{ $u->avatar }}" alt="" /> <span class="user-avatar">{{ $u->username }} <i class="ti-angle-down f-s-10"></i></span>
